  ...
  <v-container class="pl46 entire">
    <v-layout row wrap>
      <v-flex xs12 md8 class="fix-card">
        ...
        <nav class="package-toolbar">
          <ul id="secondaryTab" class="toolbar-container" style="padding-left: 0px;">
            <li class="list-style">
              <v-btn flat class="btns" @click="scrollTo(1)">Overview</v-btn>
            </li>
            <li class="list-style">
...
<v-divider></v-divider>
        ...
        <section class="overview mt-20 brief" id="1">
        ...
        <v-divider></v-divider>
        <section id="2" class="amenities mt-20 mb-20 brief">
        ...
        <v-divider></v-divider>
        <section id="3" class="amenities mt-20 mb-20 brief">
        ...
        <section id="4" class="amenities mt-20 brief">
        ...
        <v-divider></v-divider>
         <section id="5" class="amenities mt-20 mb-20 brief">
...
<v-divider></v-divider>
    ...
    <v-card-actions v-else>
      ...
      <v-flex md3 class="hidden-sm-and-down" style="margin-left: 10px;">
          <BookCard top="20" :hospitalBook="hospitalPackgageInfo[0]" :scroll-y="scrollY" :countrySpecifics="countryInfo"/>
...
export default {
  ...
  async asyncData({ params, store, query, error }) {
    console.log("async data in packid")
   ...
   if (process.server) {
    ...
    return {
     ...
     console.log("async data browser")
     if(localStorage.getItem('countryCode')) {
    console.log("async localstorage set: ", store.state.userLocation.countrySpecifics.currencySymbol)
    store.commit('userLocation/reInitialiseState');
    ...
    console.log("no localstorage set")
     store.dispatch('userLocation/countryDetails');
  }
  console.log("hospitalPackgageInfo: ", this)
  console.log("countryInfo: ", store.getters['userLocation/getCountryDetails'])
  return {
       hospitalPackgageInfo: hospPackRes.data,
       countryInfo: store.getters['userLocation/getCountryDetails']
      //  countryInfo: store.getters.userLocation.getCountryDetails
    }
   }
   //console.log("hospPackRes: ", hospPackRes)
...
//  }
  ...
  mounted() {
    window.addEventListener('scroll', (event) => {
      ...
      let sections = document.getElementsByClassName('brief');
      // Array.from(sections).forEach(function(el) {
      //   if(el.classList.contains('section-pad')) {
      //     console.log("has paddings")
      //     el.classList.remove('section-pad')
      //   }
      // })
    });
...
for (var i = 0; i < btns.length; i++) {
  ...
  methods: {
    scrollTo(id) {
      ...
      if(!(elm.classList.contains('section-pad'))) {
        elm.classList.add('section-pad');
      }
